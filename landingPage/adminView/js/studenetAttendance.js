...
// Function to fetch and update attendance data based on selected class and year
async function fetchAndUpdateAttendance(className, year) {
    try {
        const response = await fetch('/api/getClassMonthlyAttendance', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ className, year })
        });
        ...
        const attendancePercentages = result.attendancePercentages || [];

        // Update chart data
        lineChart.data.datasets[0].data = attendancePercentages;
        lineChart.update();
    } catch (error) {
...
classSelection.addEventListener('change', () => {
    ...
    if (selectedClass && selectedYear) {
        fetchAndUpdateAttendance(selectedClass, selectedYear);
    }
});
...
// Initial fetch for default selected class and year on page load
document.addEventListener('DOMContentLoaded', () => {
    const selectedClass = classSelection.value;
    const selectedYear = yearSelection.value;
    if (selectedClass && selectedYear) {
