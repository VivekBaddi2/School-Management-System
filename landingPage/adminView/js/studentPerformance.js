...
const data = {
    ...
    datasets: [{
        label: 'Academic Performance Percentage',
...
// Function to fetch and update chart data based on selected class and subject
async function updateChartData() {
    const selectedClass = classSelection.value;
    const selectedSubject = subjectSelection.value;
    ...
    try {
        const response = await fetch('/api/getClassSubjectPerformance', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ className: selectedClass, subjectName: selectedSubject })
        });
        ...
        // Assuming backend returns averageMarks as a single value
        // For demonstration, fill all months with the same averageMarks value
        const averageMarks = data.averageMarks || 0;
        const monthlyData = new Array(12).fill(averageMarks);
...
// Add event listeners to dropdowns to update chart on change
classSelection.addEventListener('change', updateChartData);
subjectSelection.addEventListener('change', updateChartData);

// Initial chart update on page load if selections are set
document.addEventListener('DOMContentLoaded', () => {
    updateChartData();
});
...
function applyTheme(theme) {
    if (theme === 'dark') {
        ...
        subjectSelection.classList.add('dark');
        ...
        subjectSelection.classList.remove('dark');
